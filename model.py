...
class TransformerEncoderLayerPU(nn.Module):
    ...
    def forward(self, src: torch.Tensor, train_test_split_index: int) -> torch.Tensor:
        ...
        # Keep the split index in the interface for compatibility, but use
        # unrestricted row-wise self-attention so every row can attend to every row.
        _ = train_test_split_index
        src_d = self.self_attention_between_datapoints(src_d, src_d, src_d)[0] + src_d
